...
app.controller('bookingDialogCtrl', function ($scope,$localStorage,$http) {
    ...
    $scope.bookStudentsEvents=function(id){
		$localStorage.start = $scope.day.getFullYear()+"-"+($scope.day.getMonth()+1)+"-"+$scope.day.getDate()+"T"+ $scope.startTime.getHours()+":"+ $scope.startTime.getMinutes()+":"+ $scope.startTime.getSeconds();
		$localStorage.end = $scope.day.getFullYear()+"-"+($scope.day.getMonth()+1)+"-"+$scope.day.getDate()+"T"+ $scope.endTime.getHours()+":"+ $scope.endTime.getMinutes()+":"+ $scope.endTime.getSeconds();
    ...
    $http({
        method: 'POST',
        url: 'http://'+$scope.serverUrl+'/book',
        data: JSON.stringify($scope.bookObj),
        headers: {
            'Content-type': 'text/plain'
        }
    }).success(function(data, status, headers, config) {
